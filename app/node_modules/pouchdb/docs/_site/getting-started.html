...
<p><a class='h3 anchor' href='#download' name='download'>Download Assets</a></p>
...
<p>Then visit <a href="http://127.0.0.1:8000/">http://127.0.0.1:8000/</a>. If you see the following screenshot, you are good to go:</p>

<p><a href="/static/img/screenshots/todo-1.png" style="display: block; text-align: center;">
   <img src="/static/img/screenshots/todo-1.png" style="width:400px;"/>
</a></p>

<p>It&#39;s also a good idea to open your browser&#39;s console so you can see any errors or confirmation messages.</p>

<div id='installing_pouchdb'></div>

<p><a class='h3 anchor' href='#installing_pouchdb' name='installing_pouchdb'>Installing PouchDB</a></p>

<p>Open <code>index.html</code> and include PouchDB in the app by adding a script tag:</p>

<div class="highlight"><pre><code class="language-html" data-lang="html"><span class="nt">&lt;script </span><span class="na">src=</span><span class="s">&quot;//cdn.jsdelivr.net/pouchdb/3.0.1/pouchdb.min.js&quot;</span><span class="nt">&gt;&lt;/script&gt;</span>
...
<p>PouchDB is now installed in your app and ready to use! (In production, you should use a local copy of the script.)</p>

<div id='creating_a_database'></div>

<p><a class='h3 anchor' href='#creating_a_database' name='creating_a_database'>Creating a database</a></p>
...
<p>You don&#39;t need to create a schema for the database. After giving it a name, you can immediately start writing objects to it.</p>

<div id='write_todos_to_database'></div>

<p><a class='h3 anchor' href='#write_todos_to_database' name='write_todos_to_database'>Write todos to the database</a></p>
...
<p>In PouchDB each document is required to have a unique <code>_id</code>. Any subsequent writes to a document with the same <code>_id</code> will be considered updates. Here we are using a date string as an <code>_id</code>. For our use case, it will be unique, and it can also be used to sort items in the database. You can use <code>db.post()</code> if you want random ids. The <code>_id</code> is the only thing required when creating a new document. The rest of the object you can create as you like.</p>

<p>The <code>callback</code> function will be called once the document has been written (or failed to write). If the <code>err</code> argument is not null, then it will have an object explaining the error, otherwise the <code>result</code> will hold the result.</p>

<div id='show_database_items'></div>

<p><a class='h3 anchor' href='#show_database_items' name='show_database_items'>Show items from the database</a></p>
...
<p>Once you have included this code, you should be able to refresh the page to see any todos you have entered.</p>

<div id='update_the_ui'></div>

<p><a class='h3 anchor' href='#update_the_ui' name='update_the_ui'>Update the UI</a></p>
...
<p>So every time an update happens to the database, we redraw the UI to show the new data. The <code>live</code> flag means this function will continue to run indefinitely. Now try entering a new todo and it should appear immediately.</p>

<div id='edit_a_todo'></div>

<p><a class='h3 anchor' href='#edit_a_todo' name='edit_a_todo'>Edit a todo</a></p>
...
<p>This is similiar to creating a document, however the document must also contain a <code>_rev</code> field (in addition to <code>_id</code>), otherwise the write will be rejected. This ensures that you dont accidently overwrite changes to a document.</p>

<p>You can test that this works by checking a todo item and refreshing the page. It should stay checked.</p>

<div id='delete_an_object'></div>

<p><a class='h3 anchor' href='#delete_an_object' name='delete_an_object'>Delete an object</a></p>
...
<p>Similiar to editing a document, both the <code>_id</code> and <code>_rev</code> properties are required. You may notice that we are passing around the full object that we previously read from the database. You can of course manually construct the object, like: <code>{_id: todo._id, _rev: todo._rev}</code>, but passing around the existing object is usually more convenient and less error prone.</p>

<div id='complete_todo_ui'></div>

<p><a class='h3 anchor' href='#complete_todo_ui' name='complete_todo_ui'>Complete rest of the Todo UI</a></p>
...
<div id='installing_couchdb'></div>

<p><a class='h3 anchor' href='#installing_couchdb' name='installing_couchdb'>Installing CouchDB</a></p>
...
<div id='enabling_cors'></div>

<p><a class='h3 anchor' href='#enabling_cors' name='enabling_cors'>Enabling CORS</a></p>
...
<div id='basic_two_way_sync'></div>

<p><a class='h3 anchor' href='#basic_two_way_sync' name='basic_two_way_sync'>Implement basic two way sync</a></p>
...
<p>Then we can implement the sync function like so:</p>
...
<p><code>db.replicate()</code> tells PouchDB to transfer all the documents <code>to</code> or <code>from</code> the <code>remoteCouch</code>. This can either be a string identifier or a PouchDB object. We call this twice: once to receive remote updates, and once to push local changes. Again, the <code>live</code> flag is used to tell PouchDB to carry on doing this indefinitely. The callback will be called whenever this finishes. For live replication, this will mean an error has occured, like losing your connection or you canceled the replication.</p>

<p>You should be able to open <a href="http://127.0.0.1:8000">the todo app</a> in another browser and see that the two lists stay in sync with any changes you make to them. You may also want to look at your CouchDB&#39;s Futon administration page and see the populated database.</p>

<div id='congratulations'></div>

<p><a class='h3 anchor' href='#congratulations' name='congratulations'>Congratulations!</a></p>
